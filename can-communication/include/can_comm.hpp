 ...
 */
inline void can_init_500k_accept_all() {
 ...
 */
inline bool can_send(uint32_t id, const uint8_t* data, uint8_t len,
  ...
  const esp_err_t err = twai_transmit(&m, pdMS_TO_TICKS(timeout_ms));
  return err == ESP_OK;
}
 ...
 * @return true if frame was received and copied to @p out.
 */
inline bool can_recv(CanMsg& out, uint32_t timeout_ms = 0) {
  twai_message_t rx = {};
